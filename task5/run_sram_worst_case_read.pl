...
# Final project 
#
# Determines the lowest operating voltage an sram array can run
...
# Finally, the deck must measure the maximum value reached by node xctl.bit_b, named
# max_bit_b (.meas TRAN max_bit_b MAX v(xctl.bit_b)). The spice deck must run on
...
while(<WCFILE>){
    ...
    elsif($_ =~ m/^.meas\s+TRAN\s+max_bit_b\s+MAX\s+v\(xctl.bit_b\)/i){
	$found2 = 1;
    }
    elsif($_ =~ m/^.ic\s+v\(xctl.bit\)\s*=\s*'supply'/i){
	$found3 = 1;
    }
    elsif($_ =~ m/^.ic\s+v\(xctl.bit_b\)\s*=\s*0/i){
	$found4 = 1;
    }
...
$usr_file = "./run_sram_worst_case_read.log";
...
while(!$done){
    ...
    $gen_sp = "sram_worst_case_read.sp";
	...
	if($_ =~ m/^.include '\/usr\/class\/ee313\/ee313_spice_header.h'/){
	    ...
	    print TEMPFILE ".lib '/usr/class/ee313/lib/opConditions.lib' TTTT\n";
    ...
    #iterate over the output .lis file and check max_bit_b to detect destructive read
    $dreads = 0; $val_int = 0;
    while(<TEMPFILE>){
	if($_ =~ m/(.*)(max_bit_b=\s*)([\.\dE-]*)(\s*)(.*)/){
		if($_ =~ m/(.*)(max_bit_b=\s*)([\.\dE-]*)m(\s*)(.*)/){
		    $val_int = $3;
		} else {
			print "WARNING: Units are not mV. Comparison may be incorrect";
		}
	}	
	if($val_int/1000 > (5/6*$vdd)){
	    $dreads = 1;
	    last;
	}
    }
    ...
    #print info for user
    if($dreads == 1){
	print USRFILE "Destructive read at ".$vdd."V. (max_bit_b = ".$val_int."mV)\n";
	print "Destructive read at ".$vdd."V. (max_bit_b = ".$val_int."mV)\n";
    } else {
	print USRFILE "Pass read at ".$vdd."V. (max_bit_b = ".$val_int."mV)\n";
	print "Pass read at ".$vdd."V. (max_bit_b = ".$val_int."mV)\n";
    }
